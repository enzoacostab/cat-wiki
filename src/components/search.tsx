...
export default function Search() {
  ...
  return (
    <div className={`sm:w-[60%] lg:w-full ${searchOpen && window.innerWidth<640 ? 'fixed top-0 left-0 size-full mt-0 z-20 bg-white' : 'mt-[6%]'}  text-black sm:relative text-xs lg:text-lg`}>
      ...
      <div className={`bg-white absolute w-full sm:h-fit px-4 py-4 rounded-3xl transition-all ${search ? 'sm:opacity-100 sm:top-[120%] sm:z-20' : 'sm:opacity-0 sm:top-0 left-0 sm:-z-10'} ${searchOpen && window.innerWidth<640 ? 'h-full z-20' : 'opacity-0 h-0 -z-20'}`}>
